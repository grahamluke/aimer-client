...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  function genCustomUsername() {
    let clan;
    checkElement('.menuClassPlayerClan').then((selector) => {
    clan = selector.textContent
    })
    checkElement('#leaderContainer').then((selector) => {
    new MutationObserver(() => {
      for (const i of document.querySelectorAll('.leaderNameM')) {
        if(i.textContent == window.getGameActivity().user +` ${clan}`){
          i.textContent = config.get('customUsername') +` ${clan}`
        }}
    ...
    // Fix this so it always show on endTable: setTimeout(checkElement('#endTable').then(arguments.callee()), 20000)
    });
  }
...
function initTwitch() {
    ...
    client.on('message', (channel, tags, message, self) => {
        document.getElementById('chatList').insertAdjacentHTML('beforeend', `<div id="chatMsg_Twitch"><div class="chatItem chatTextOutline twitch" style="background-color: rgba(0, 0, 0, 1)">&lrm;${tags['display-name']}&lrm;: <span class="chatMsg">&lrm;${message}&lrm;</span></div><br></div>`)
    });
  ...
  const injectSettings = () => {
    ...
    window.setClientSetting = function (setting) {
      if (setting == 'customCSS' || setting == 'customUsername' || setting == 'twitch' || setting == 'loadingScreenImage') {
    ...
    function getClientSetting(setting) {
      ...
      } else if(setting === 'loadingScreenImage') {
        window[`${setting}`] = config.get('loadingScreenImage')
      }
    ...
    windows[0].getCSettings = () => {
      ...
      tempHTML += `<div class="settName" id="twitch_div" style="display:block">Twitch<input type="text" placeholder="Enter Twitch Channel Name Here" name="text" class="inputGrey2" id="twitch" oninput=setClientSetting("twitch") value="${twitch}"></div>`
      tempHTML += `<div class="settName" id="loadingScreenImage_div" style="display:block">Loading Screen Image<input type="text" placeholder="Enter Loading Screen Image Link Here" name="text" class="inputGrey2" id="loadingScreenImage" oninput=setClientSetting("loadingScreenImage") value="${loadingScreenImage}"></div>`
      return tempHTML
    }
  }
  function init() {
    ...
    badges()
    injectSettings()
    if(config.get('customUsername') !== '') genCustomUsername()
